...
extern GLuint texturaHierba, texturaAbajoSkybox, texturaPista, texturaArribaSkybox, texturaDerechaSkybox, texturaIzquierdaSkybox, skyboxNor, skyboxSur, texturaAvion;
extern objeto sol, mercurio, venus, tierra, luna, iss, marte, jupiter, saturno, urano, neptuno;

GLuint listaSuelo, listaArcos;
...
void crearAvion() {
	...
	/// Main body
	glColor3d(0.0, 0, 0);
	glBindTexture(GL_TEXTURE_2D, texturaAvion);
	glPushMatrix();
	glTranslated(0, 0, 0);
	glScaled(3, 0.4, 0.5);
...
void dibujarAvion(avion *p) {
	...
	glTranslatef(p->posicionX, p->posicionY, p->posicionZ);
	glRotatef(p->angulo, 0, 1, 0);
	glRotatef(p->angulo2, 1, 0, 0);
	glRotatef(p->angulo3, 0, 0, 1);

	glBindTexture(GL_TEXTURE_2D, texturaAvion);
	glScalef(1.0f, 1.0f, 1.0f);
	glRotatef(90.0f, 0, 1, 0);
	glTranslatef(0, p->altura, 0);
	crearAvion();	
...
void dibujarCajaTexto(laser x) {
	...
	glPushMatrix();
		...
		glTranslatef(x.px, x.py, x.pz);
		glRotatef(x.angulo1, 0, 1, 0);
		glRotatef(x.angulo2, 1, 0, 0);
		glPushMatrix();
			glScalef(10.0, 10.0, 0.001);
			glCallList(x.listarender);
		glPopMatrix();
	glPopMatrix();
}
...
void skybox(void) {
	...
	glBindTexture(GL_TEXTURE_2D, skyboxSur);

	glBegin(GL_QUADS);
	glTexCoord2f(0.0f, 1.0f);
	glVertex3f(N*-4000.0f, 4000.0*N, 4000.0*N);
	...
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindTexture(GL_TEXTURE_2D, texturaIzquierdaSkybox);
	...
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindTexture(GL_TEXTURE_2D, skyboxNor);
	...
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindTexture(GL_TEXTURE_2D, texturaDerechaSkybox);
	...
	//Cara 5 (superior)
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindTexture(GL_TEXTURE_2D, texturaArribaSkybox);
	...
	//Cara 6 (inferior)
	glBindTexture(GL_TEXTURE_2D, 0);
	glBindTexture(GL_TEXTURE_2D, texturaAbajoSkybox);
...
;
	glPushMatrix();
		glColor3ub(255, 0, 0);
		glTranslatef(ls.px, ls.py, ls.pz);
		glRotatef(ls.angulo1, 0, 1, 0);
		glRotatef(ls.angulo2, 1, 0, 0);
		glPushMatrix();
			glScalef(1.0/n, 2.0/n, 25.0/n);
			glCallList(ls.listarender);
		glPopMatrix();
	glPopMatrix();
}
...
void dibujarPlaneta(objeto object) {
	glPushMatrix();
	/*glRotatef(object.angulo_trans, 0.0, 1.0, 0.0);
	glTranslatef(object.distancia, 0.0, 0.0);*/
	glTranslatef(object.px, 0.0, object.pz);
	glPushMatrix();
	glRotatef(object.angulo_rot, 0, 1, 0);
	glScalef(object.tamano, object.tamano, object.tamano);
	glBindTexture(GL_TEXTURE_2D, object.textura);
	glCallList(object.listaRender);
	glPopMatrix();
	glPopMatrix();
}
void dibujarISS() {
	//Satélite
	glPushMatrix();
	glTranslatef(iss.px, -24.7, iss.pz);
	glPushMatrix();
...
void dibujarEsquinaControles() {
	...
	double *matrix;
	std::string text = "VER CONTROLES: 5";
	length = text.size();
	x = 7;
	y = 10;
